...
inline int32_t _mm512_reduce_add_epi16(__m512i vec) {
    // 将512位向量拆分为两个256位向量
    __m256i vec_low = _mm512_extracti64x4_epi64(vec, 0);  // 低256位
    __m256i vec_high = _mm512_extracti64x4_epi64(vec, 1); // 高256位
    
    // 两个256位向量相加 (每个包含16个16位整数)
    __m256i sum256 = _mm256_add_epi16(vec_low, vec_high);
    
    // 将256位向量拆分为两个128位向量
    __m128i sum128_low = _mm256_extracti128_si256(sum256, 0);  // 低128位
    __m128i sum128_high = _mm256_extracti128_si256(sum256, 1); // 高128位
    
    // 两个128位向量相加 (每个包含8个16位整数)
    __m128i sum128 = _mm_add_epi16(sum128_low, sum128_high);
    
    int16_t dst[8];
    _mm_storeu_si16(dst, sum128);
    int32_t ans=0;
    for (int i=0; i<8; ++i) ans+=dst[i];
    return ans;
}
...
void matmul(float* xout, QuantizedTensor *x, QuantizedTensor *w, int n, int d) {
    ...
    for (i=0; i<d; ++i) {
        ...
        for (int g=0; g<n/GS; ++g) {
            ...
            for (int j=0; j<GS; j+=32) {
                /*
                float wij = w->q[i*n+g*GS+j] * swij;
                float xj = x->q[g*GS+j] * sxj;
                xout[i]+=wij*xj;*/
                __m256i w_vec_int8 = _mm256_loadu_si256((__m256i*)&w->q[i * n + g * GS + j]);
                __m256i x_vec_int8 = _mm256_loadu_si256((__m256i*)&x->q[g * GS + j]);
                __m512i w_vec_int16 = _mm512_cvtepi8_epi16(w_vec_int8);
                __m512i x_vec_int16 = _mm512_cvtepi8_epi16(x_vec_int8);
                __m512i w_x_int16 = _mm512_mullo_epi16(w_vec_int16, x_vec_int16);
                /*
                __m512 wVec = _mm512_cvtepi32_ps(w_vec_int32);
                __m512 xVec = _mm512_cvtepi32_ps(x_vec_int32);
                // __m512 wVec = _mm512_loadu_ps(&w->q[i*n+g*GS+j]);
                __m512 swijV = _mm512_set1_ps(swij);
                wVec = _mm512_mul_ps(wVec, swijV);
                // __m512 xVec = _mm512_loadu_ps(&x->q[g*GS+j]);
                __m512 sxjV = _mm512_set1_ps(sxj);
                xVec = _mm512_mul_ps(xVec, sxjV);
                __m512 acc = _mm512_mul_ps(wVec, xVec);
                */
                int ans =  _mm512_reduce_add_epi16(w_x_int16);
                xout[i] += ans*s;
            }
        }
    }
