...
void malloc_run_state(RunState* s, Config* p) {
    ...
    s->x = aligned_alloc(alignment, ALIGNED_SIZE(x_size));
    // 分配并初始化s->xb
    size_t xb_size = p->dim * sizeof(float);
    s->xb = aligned_alloc(alignment, ALIGNED_SIZE(xb_size));
    // 分配并初始化s->xb2
    size_t xb2_size = p->dim * sizeof(float);
    s->xb2 = aligned_alloc(alignment, ALIGNED_SIZE(xb2_size));
    // 分配并初始化s->hb
    size_t hb_size = p->hidden_dim * sizeof(float);
    s->hb = aligned_alloc(alignment, ALIGNED_SIZE(hb_size));
    // 分配并初始化s->hb2
    size_t hb2_size = p->hidden_dim * sizeof(float);
    s->hb2 = aligned_alloc(alignment, ALIGNED_SIZE(hb2_size));
    // 分配并初始化s->xq
    size_t xq_q_size = p->dim * sizeof(int8_t);
    s->xq.q = aligned_alloc(alignment, ALIGNED_SIZE(xq_q_size));
    size_t xq_s_size = p->dim * sizeof(float);
    s->xq.s = aligned_alloc(alignment, ALIGNED_SIZE(xq_s_size));
    // 分配并初始化s->hq
    size_t hq_q_size = p->hidden_dim * sizeof(int8_t);
    s->hq.q = aligned_alloc(alignment, ALIGNED_SIZE(hq_q_size));
    size_t hq_s_size = p->hidden_dim * sizeof(float);
    s->hq.s = aligned_alloc(alignment, ALIGNED_SIZE(hq_s_size));
    // 分配并初始化s->q
    size_t q_size = p->dim * sizeof(float);
    s->q = aligned_alloc(alignment, ALIGNED_SIZE(q_size));
    // 分配并初始化s->k
    size_t k_size = kv_dim * sizeof(float);
    s->k = aligned_alloc(alignment, ALIGNED_SIZE(k_size));
    // 分配并初始化s->v
    size_t v_size = kv_dim * sizeof(float);
    s->v = aligned_alloc(alignment, ALIGNED_SIZE(v_size));
    // 分配并初始化s->att
    size_t att_size = p->n_heads * p->seq_len * sizeof(float);
    s->att = aligned_alloc(alignment, ALIGNED_SIZE(att_size));
    // 分配并初始化s->logits
    size_t logits_size = p->vocab_size * sizeof(float);
    s->logits = aligned_alloc(alignment, ALIGNED_SIZE(logits_size));
    // 分配并初始化s->key_cache
    size_t key_cache_size = p->n_layers * p->seq_len * kv_dim * sizeof(float);
    s->key_cache = aligned_alloc(alignment, ALIGNED_SIZE(key_cache_size));
    // 分配并初始化s->value_cache
    size_t value_cache_size = p->n_layers * p->seq_len * kv_dim * sizeof(float);
    s->value_cache = aligned_alloc(alignment, ALIGNED_SIZE(value_cache_size));
...
void rmsnorm(float* o, float* x, float* weight, int size) {
    ...
    // 归一化并乘以缩放系数
    for (int i=0; i<size; ++i)
        o[i] = weight[i]*x[i]/(rms+eps);
}

void softmax(float* x, int size) {
    // 找到最大值（数值稳定性）
    float m = __FLT_MIN__;
    for (int i=0; i<size; ++i) m = fmax(m, x[i]);
    // 计算指数并求和
    float sum = 0.f;
    for (int i=0; i<size; ++i) sum+=exp(x[i]-m);
    // 再次归一化得到概率分布
    for (int i=0; i<size; ++i) x[i] = exp(x[i]-m)/sum;
}
